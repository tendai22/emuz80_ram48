...
#define RAM_SIZE ((addr_t)0xffff) // RAM size 4K bytes
#define RAM_TOP ((addr_t)0x0000) // RAM start address
#define UART_DREG ((unsigned long)0x01) // UART data register address
#define UART_CREG ((unsigned long)0x00) // UART control register address
//#define HALT_REG  ((unsigned long)0xE002) // System HALT activate
#define DBG_PORT ((unsigned long)0x80) // debug port address DBG_PORT
